import Joi from "joi";
export default function CreateCategoriesMiddleware(req, res, next) {
  const data = req.body; // lấy dũ liệu từ người dùng gửi lên
  const schema = Joi.object({
    name: Joi.string().max(255).required().messages({
      "string.base": "Tên categories là 1 chuỗi.",
      "string.max": "Tên categories không được vượt quá {{#limit}} ký tự.",
      "any.required": "Vui lòng nhập tên categories của bạn.",
    }),
